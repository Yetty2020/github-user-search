...
/**
 * Calculates the top trending GitHub users
 * Trending score is based on followers and repository count
 *
 * @param users Array of user data from GitHub GraphQL API
 * @returns Array of users sorted by trending score (highest first)
 */

export const calculateTrendingUsers = (
  users: TrendingUser[]
): (TrendingUser & { score: number })[] => {
  if (users.length === 0) {
    return [];
  }
